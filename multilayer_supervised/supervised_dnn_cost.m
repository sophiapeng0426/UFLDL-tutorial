...
hAct = cell(numHidden+1, 1); % zi for each layer
gradStack = cell(numHidden+1, 1);
%% forward prop
%%% YOUR CODE HERE %%%
for i=1:numHidden+1
    if i == 1
        hAct{1} = bsxfun(@plus,stack{1}.W*data,stack{1}.b); % Wx+b
    else
        hAct{i} = bsxfun(@plus,stack{i}.W*hAct{i-1},stack{i}.b);
    end
    
    % non-linear transformation for hidden layer
    if i < numHidden+1
